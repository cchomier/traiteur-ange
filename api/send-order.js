...
function blocReponseClient({ ref, order }) {
  ...
  // v2 : le manager renvoie un devis modifie a la demande du client. Le message
  // le dit clairement et invite a consulter la piece jointe, au lieu du simple
  // accuse de reception d'une premiere demande.
  const estModif = Number(o.version) > 1;

  const objet = estModif
    ? `Votre devis traiteur modifié${ref ? " " + ref : ""}`
    : `Votre demande traiteur${ref ? " " + ref : ""} — bien reçue`;

  // \r\n et non \n : Outlook et Mail iOS ignorent le simple saut de ligne dans un
  // mailto et collent tout le message sur une seule ligne.
  const corps = estModif
    ? [
        `Bonjour${prenom ? " " + prenom : ""},`,
        "",
        `Suite à votre demande, voici votre devis traiteur modifié${quand ? " pour le " + quand : ""}${heure ? " à " + heure : ""}, en pièce jointe.`,
...
async function logToSupabase({ ref, order }) {
  ...
  const row = {
    ...
    // Versionnage v2 : base_ref regroupe toutes les versions d'un meme devis,
    // version = 1, 2, 3... Un devis neuf est sa propre base en version 1.
    base_ref: o.baseRef || o.ref || null,
    version: Number.isInteger(o.version) ? o.version : 1,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Sujet : prefixe [DEVIS MODIFIE] quand c'est une v2, pour que la boulangerie
    // reconnaisse tout de suite une reprise et ne la traite pas comme une demande neuve.
    const estModif = order && Number(order.version) > 1;
    const payload = {
      from: FROM,
      to,
      subject: `${estModif ? "[DEVIS MODIFIE] " : ""}Devis traiteur${bakeryName ? " [" + bakeryName + "]" : ""} ${ref || ""} — ${clientName || "client"}`,
