...
// Ne renvoie QUE ref + lignee + panier : jamais toute la ligne. Acces par jeton non devinable.
//
...
export default async function handler(req, res) {
  ...
  try {
    // select limite : ref + lignee + panier. La route ne fuit jamais le reste de la ligne.
    const q = `${url}/rest/v1/${SB_TABLE}?reprise_token=eq.${encodeURIComponent(token)}&select=ref,base_ref,version,panier&limit=1`;
    ...
    return res.status(200).json({ ref: row.ref, base_ref: row.base_ref || row.ref, version: row.version || 1, panier: row.panier });
  } catch (err) {
